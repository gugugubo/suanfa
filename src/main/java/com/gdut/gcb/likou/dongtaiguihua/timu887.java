...
import com.gdut.gcb.likou.shaungzhizhen.timu88;
 ...
 * @Version 1.0
 **/
public class timu887 {

    public int superEggDrop(int K, int N) {
        // dp[i][j]表示i个鸡蛋扔j层楼梯时所需的最小的步数
        ...
        for(int i=2;i<=K;i++){
            for (int j=2;j<=N;j++){
                // 遍历第一个鸡蛋从1到J层的时候j层最坏情况下的最小步数
                // 然后取各层的最小值
                int min  = 999;
                for (int f = 1;f<=j;f++){
                    // 里面使用max是因为这个鸡蛋碎没碎是不确定的，所有需要考虑最坏情况下的值
                    min = Math.min(min , Math.max(dp[i-1][f-1] , dp[i][j-f]));
                }
                dp[i][j] = min + 1;
            }
        }
    ...
    public static void main(String[] args) {
        timu887 test = new timu887();
        long start1 = System.currentTimeMillis();
        int i = test.superEggDrop(9, 1000);
        System.out.println(i);
        System.out.println(System.currentTimeMillis() -start1);

        long start2 = System.currentTimeMillis();
        int i2 = test.superEggDrop2(9, 1000);
        System.out.println(i2);
        System.out.println(System.currentTimeMillis() -start2);
        
        int i1 = test.superEggDrop(1, 2);
